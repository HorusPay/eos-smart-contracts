...
namespace horuspaytoken {
   ...
   void horustokenio::change_resource( account_name from, account_name receiver,
      ...
      if ( stakingaccount != source_stake_from ) { //for stakingaccount both transfer and refund make no sense
         ...
         if ( asset(0, HORUS_SYMBOL) < transfer_amount ) {
            INLINE_ACTION_SENDER(horustokenio, transfer)( code, {source_stake_from, N(active)},
               { source_stake_from, stakingaccount/* change this to the account accepting the stake*/, asset(transfer_amount), std::string("staking HORUS") } );
         }
      }
   }
   ...
   void horustokenio::refundhorus( const account_name owner ) {
      ...
      INLINE_ACTION_SENDER(horuspaytoken::horustokenio, transfer)( code, {stakingaccount,N(active)},
                                                    { stakingaccount, req->owner, req->horus_amount, std::string("unstake HORUS") } );
